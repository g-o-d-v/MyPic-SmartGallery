...
Java_com_goda_mypic_OcrEngine_init(
...
Java_com_goda_mypic_OcrEngine_forward(
  ...
  try {
    ...
    // ==========================================================
    // ... (把你原本在这个函数里的所有代码，统统放在这里面) ...
    // ... 例如获取指针、转换 Mat、调用 infer_ocr、返回 result 等 ...
    // ==========================================================
    ...
    // 🚨 2. 在函数末尾加入 catch，拦截所有致命崩溃！
  catch (const cv::Exception& e) {
    LOGI("🚨 拦截到 OpenCV 崩溃，已安全跳过此图片: %s", e.what());
    return NULL;
  } catch (const std::exception& e) {
    LOGI("🚨 拦截到 C++ 标准异常，已安全跳过此图片: %s", e.what());
    return NULL;
  } catch (...) {
    LOGI("🚨 拦截到未知 C++ 异常，已安全跳过此图片");
    return NULL;
  }
}

extern "C" JNIEXPORT void JNICALL
Java_com_goda_mypic_OcrEngine_release(
